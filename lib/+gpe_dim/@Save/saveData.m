...
if isfield(params, 'Ul')
    if s == 1
        obj.Uls = zeros(sf, length(params.Ul));
    end
    obj.Uls(s, :) = params.Ul;
    Uls = obj.Uls;
    save([obj.Data 'Ul.mat'], 'Uls')
end

if isfield(params, 'Ut')
    if s == 1
        obj.Uts = zeros(sf, length(params.Ut));
    end
    obj.Uts(s, :) = params.Ut;
    Uts = obj.Uts;
    save([obj.Data 'Ut.mat'], 'Uts')
end

if isfield(params, 'mu')
    if s == 1
        obj.mus = zeros(sf, length(params.L));
    end
    obj.mus(s, :) = gather(params.mu);
    mus = obj.mus;
