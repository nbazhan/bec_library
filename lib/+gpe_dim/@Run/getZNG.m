...
if isempty(obj.config.mu)
    if obj.config.D == 3
        ...
        %phi0 = ones(obj.grid.N.x, obj.grid.N.y, obj.grid.N.z) + ...
        %            1i*ones(obj.grid.N.x, obj.grid.N.y, obj.grid.N.z); 
    elseif obj.config.D == 2
        phi0 = rand(obj.grid.N.x, obj.grid.N.y) + ...
                1i*rand(obj.grid.N.x, obj.grid.N.y);  
        %phi0 = ones(obj.grid.N.x, obj.grid.N.y) + ...
        %        1i*ones(obj.grid.N.x, obj.grid.N.y); 
    end
...
util.check(obj.test, struct('Init_V', V, ...
                            'Init_Nl2', Nl2, ...
                            'Init_ekk', ekk, ...
                            'Init_Nl', Nl), '-');
while true 
    ...
    util.check(obj.test, struct('Psi2', Psi2), '-')
    ...
    if(T > 0 && mod(i,10) == 1 && isempty(obj.config.mu)) % for better performance and stability we do some initial iterations without a thermal cloud
        ...
        Nc = obj.config.N - Nt;
        util.check(obj.test, struct('Nt', Nt, ...
                                    'Nc', Nc))
        if(Nc <= 1)
            Nc = 1;
    ...
    H = obj.applyham(Psi, Nl + 2*obj.config.g*obj.shrink(nt));
    mu2 = real(sum(H(:)).*obj.grid.dV)/N;
    MU(i) = gather(mu2);

    util.check(obj.test, struct('i', i, ...
                                'nt', nt, ...
                                'c', c, ...
                                'Psi2', Psi2, ...
                                'Nl', Nl, ...
    ...
    util.check(obj.test, struct('Nl', Nl));
   
    if(i > 50) && mod(i,10) == 5
        delta = (abs(C(i)-C(i-9))/9 + abs(C(i)-C(i-1)))/dt/C(i);
        util.check(obj.test, struct('delta', delta))
        if(delta < eps)
