...
%phi = sqrt(NN0)*grid.normalize(ones(size(grid.mesh.x),'like',V) + 1i*ones(size(grid.mesh.x),'like',V)); % random initial guess
...
util.check(task.test, struct('Init_V', V, ...
                            'Init_Nl2', tmpext, ...
                            'Init_ekk', ekk, ...
                            'Init_Nl', tmp2), '-');
while true %(delta > eps || mod(i,10)~=5)
    ...
    util.check(task.test, struct('Psi2', tmp), '-')
    ...
    if(TT>0 && mod(i,10)==1 && task.Ntotal > 0) % for better performance and stability we do some initial iterations without a thermal cloud
        ...
        NNN = NN0 - NNt;
        util.check(task.test, struct('Nt', NNt, ...
                                    'Nc', NNN))
        if(NNN<1)
            NNN=1;
    ...
    MU2(i) = MU2(i)/ncur;
    util.check(task.test, struct('i', i, ...
                                'nt', nt, ...
                                'c', MU(i), ...
                                'Nl', tmp2, ...
                                'mu', MU2(i)));
    if(mod(i,10)==0)%i>150)
        if(TT>0)
            ...
            ntt=(TT/(2*pi))^1.5*util_yb.mypolylog(1.5,exp((MU2(i) - tmpext - 2*g*nt)/TT)); % averaging increases stability for high temperatures
    ...
    util.check(task.test, struct('Nl', tmp2));

    if(i>50 && mod(i,10) == 5)
        delta = (abs(MU(i)-MU(i-9))/9 + abs(MU(i)-MU(i-1)))/dt/MU(i);
        util.check(task.test, struct('delta', delta))
        if(delta < eps)
