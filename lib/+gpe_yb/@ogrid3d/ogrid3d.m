...
%%  Properties
  properties
	omx         % harmoer nic trap frequency in x direction
...
%         obj.wz = obj.wz.*(obj.wz>1e-30);
    ...
    function [x,w] = gauss_hermite_wrap(obj,n,alpha)
        [x,w] = util_yb.gauss_hermite(n);
        x = (x./sqrt(alpha));
        w = (w./sqrt(alpha));
    end
	function u=osc_state(obj,n,x,om)
        [~, xgrid] = meshgrid(n,x);
		u = util_yb.hermite(n,x.*sqrt(om),'norm').*exp(-om*xgrid.^2*0.5)*om^0.25; %./sqrt(2^n*factorial(n)*sqrt(pi/om));
    end
